...
export async function renderPDF(data: ReportData): Promise<Buffer> {
  return new Promise<Buffer>((resolve, reject) => {
    try {
      ...
      // --- AI Analysis ---
      if (data.aiAnalysis) {
        doc.fontSize(14).font('Helvetica-Bold').text('AI Analysis & Recommendations');
        doc.moveDown(0.5);
        doc.fontSize(10).font('Helvetica').text(data.aiAnalysis, {
          lineGap: 4,
          paragraphGap: 4,
        });
        doc.moveDown(0.5);
        doc.fontSize(8).fillColor('#888888').text('Generated by SaathiAI — verify findings before submission.');
...
async function ensureBucket(): Promise<void> {
  ...
  if (listErr) {
    logger.warn({ error: listErr }, 'Could not list storage buckets; skipping auto-create');
    return;
  }
  const exists = (buckets ?? []).some((b: { name: string }) => b.name === STORAGE_BUCKET);
  if (!exists) {
    const { error: createErr } = await db.storage.createBucket(STORAGE_BUCKET, {
      public: false,
      allowedMimeTypes: ['application/pdf', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'],
      fileSizeLimit: 52428800, // 50 MB
    });
    if (createErr) {
      logger.error({ error: createErr }, `Failed to auto-create '${STORAGE_BUCKET}' bucket`);
      throw new Error(`Storage bucket '${STORAGE_BUCKET}' missing and could not be created: ${createErr.message}`);
    }
    logger.info(`Auto-created storage bucket '${STORAGE_BUCKET}'`);
  }
}
 ...
 * and return a signed URL with 24-hour expiry. Auto-creates the bucket if missing.
 *
